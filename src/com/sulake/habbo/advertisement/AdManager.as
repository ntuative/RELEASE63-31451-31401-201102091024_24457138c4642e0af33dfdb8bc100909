package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_1031:BitmapData = null;
      
      private var var_1032:BitmapData = null;
      ...
      {
         this.var_293 = param2 as IHabboCommunicationManager;
         if(this.var_293 == null)
         {
            return;
         }
         this._connection = this.var_293.getHabboMainConnection(this.onConnectionReady);
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2236 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2238 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2235 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1592 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_204 != null && this.var_204.running)
         {
            this.var_204.reset();
            return false;
         }
         if(this.var_1033 == 0)
         {
            this.var_1033 = getTimer();
            return false;
         }
         if(this.var_1592 > 0 && this.var_1591 >= this.var_1592)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1033;
         if(_loc1_ > this.var_2238)
         {
            ++this.var_1591;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         if(this.var_1297 != "")
         {
            ...
            {
               this.var_204 = new Timer(this.var_2235,1);
               this.var_204.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_204.running)
            {
               this.var_204.reset();
            }
            this.var_204.start();
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_421,0,0,param1.bitmapData,this.var_2237));
            this.var_1033 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_475));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1296 = _loc2_.imageUrl;
         this.var_1590 = _loc2_.clickUrl;
         if(this.var_1296 != "" && this.var_1590 != "")
         {
            _loc3_ = this.var_1296;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1296);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_539,0,0,_loc4_,this.var_1590,this.var_1031,this.var_1032));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1031 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1032 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
