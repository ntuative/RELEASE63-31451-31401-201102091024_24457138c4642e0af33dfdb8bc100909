package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_403:AssetLoaderStruct;
      ...
      private const const_1958:int = 100;
      
      private const const_1382:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_403 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_877 = new Timer(this.const_1958,1);
         this.var_877.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_403)
         {
            this.var_403.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_403.addEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_403)
         {
            this.var_403.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_403.removeEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1344.indexOf("?") > 0)
            {
               _loc2_ = this.var_1344 + "&retry=" + this.var_878;
            }
            else
            {
               _loc2_ = this.var_1344 + "?retry=" + this.var_878;
            ...
            this.var_403 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1722 = true;
         for each(_loc2_ in this.var_1082)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2425,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1083[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1083[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1722)
         {
            this.var_1082.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_490[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_490[_loc3_] = _loc6_;
            }
            this.var_876[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_876)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_490[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_876[_loc3_];
         }
         while(_loc6_ < this.var_719.length)
         {
            _loc5_ = this.var_719[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_719.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_877.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_720.length > 0 && this.var_719.length < this.const_1382)
         {
            _loc1_ = this.var_720[0];
            _loc1_.startDownloading();
            this.var_719.push(this.var_720.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_720.indexOf(param1) == -1 && this.var_719.indexOf(param1) == -1)
         {
