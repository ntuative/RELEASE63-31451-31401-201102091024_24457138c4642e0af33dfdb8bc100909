package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_178.isActive)
            {
               _loc2_ = this.var_178.secondsLeft / Number(this.var_178.duration);
               this.var_911.width = _loc2_ * this.var_273;
            }
            else
            {
               this.var_911.width = 0;
               this.var_193.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_913.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_913.text;
            if(this.var_178.isActive)
            {
               _loc2_ = this.var_178.secondsLeft;
               ...
               this.var_913.text = _loc1_;
            }
            else
            {
               this.var_913.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_178.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_178.isActive)
         {
         ...
         else
         {
            ...
            this.var_912 = this._window.findChildByName("effect_hilite");
            this.var_912.visible = false;
         }
         this.var_913 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_911 = this._window.findChildByName("loader_bar");
         if(this.var_911 != null)
         {
            this.var_273 = this.var_911.width;
            this.var_193.start();
            this.onUpdate();
         }
         if(this.var_178.icon)
         {
            this.setElementBitmap("effect_icon",this.var_178.icon);
         }
      }
