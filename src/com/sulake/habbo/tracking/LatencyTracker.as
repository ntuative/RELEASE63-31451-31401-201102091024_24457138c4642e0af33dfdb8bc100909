package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_1643 = int(this.var_182.getKey("latencytest.interval"));
         this.var_1640 = int(this.var_182.getKey("latencytest.report.index"));
         this.var_2322 = int(this.var_182.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1641 > this.var_1643)
         {
            this.testLatency();
         }
      }
      ...
      private function testLatency() : void
      {
         ...
         this.var_565.add(this.var_1318,this.var_1641);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1318));
         ++this.var_1318;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_565 == null || this.var_205 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_565.getValue(_loc2_.requestId);
         this.var_565.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_205.push(_loc4_);
         if(this.var_205.length == this.var_1640 && this.var_1640 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1642) > this.var_2322 || this.var_1642 == 0)
            {
               this.var_1642 = _loc9_;
