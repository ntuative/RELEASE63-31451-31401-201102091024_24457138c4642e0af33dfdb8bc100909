package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_429:String = "InitConnection";
       
      
      private var var_159:HabboCommunicationDemo;
      ...
      {
         this.var_159.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_159 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_159.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_147,HabboWindowStyle.const_40,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_159.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         if(_loc8_)
         {
            if(this.var_159.habboConfiguration)
            {
               if(this.var_159.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_159.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_159.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_159.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_133.removeEventListener(SSOTicket.const_392,this.onParsedTicketSuccess);
            this.var_133.removeEventListener(SSOTicket.const_306,this.onParsedTicketFailure);
            this.var_133 = null;
         }
         var _loc2_:String = "";
         if(this.var_159.habboConfiguration != null)
         {
            _loc2_ = this.var_159.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_159.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_133 = new SSOTicket(this.var_159.assets,_loc3_,_loc2_,param1);
         this.var_133.addEventListener(SSOTicket.const_392,this.onParsedTicketSuccess);
         this.var_133.addEventListener(SSOTicket.const_306,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_159.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_159.shockwaveClientUrl = this.var_133.shockwaveClientUrl;
         this.var_159.ssoTicket = this.var_133.ticket;
         this.var_133.removeEventListener(SSOTicket.const_392,this.onParsedTicketSuccess);
         this.var_133.removeEventListener(SSOTicket.const_306,this.onParsedTicketFailure);
         this.var_133 = null;
         dispatchEvent(new Event(const_429));
